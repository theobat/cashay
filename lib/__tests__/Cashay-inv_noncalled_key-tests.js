  ...
  return _Cashay2.default.query(getRecentPostsQuery, {
    ...
    mutationHandlers: {
      removePostById: function removePostById(optimisticVariables, queryResponse, currentResponse, getEntities, invalidate) {
        if (optimisticVariables) return;
        if (queryResponse) invalidate();
      }
    }
  });
}

function removePostByIdCashay(postId) {
  return _Cashay2.default.mutate('removePostById', { variables: { postId: postId }, ops: { RecentPosts: true } });
}
...
(0, _ava2.default)('postCount query and mutation invalidation', function (t) {
  ...
  return new Promise(function (resolve) {
    getRecentPostsQueryCashay();resolve();
  }).then(function () {
    return removePostByIdCashay('p124');
  }
  // ).then(
  //   () => {
  ...
  //   }
    ...
    return removePostByIdCashay('p125');
  });
});
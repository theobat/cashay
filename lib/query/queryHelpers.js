...
var shortenNormalizedResponse = exports.shortenNormalizedResponse = function shortenNormalizedResponse(normalizedResponse, cashayState) {
  var resEntityGlobalMap = normalizedResponse.entities;
  if (!resEntityGlobalMap) return;
  var resEntityAllNames = Object.keys(resEntityGlobalMap);
  var diff = _extends({}, normalizedResponse, { entities: {} });
  resEntityAllNames.forEach(function (entityName) {
    var resEntityKeyMap = resEntityGlobalMap[entityName];
    if (cashayState.entities[entityName]) {
      var resEntityKeys = Object.keys(resEntityKeyMap);
      resEntityKeys.forEach(function (key) {
        if (resEntityKeyMap[key] !== undefined && resEntityKeyMap[key] !== null && JSON.stringify(resEntityKeyMap[key]) !== JSON.stringify(cashayState.entities[entityName][key])) {
